...
function Lobby() {
  ...
  const loadLobbies = async () => {
    try {
      console.log('[Lobby] Loading lobbies...');
      ...
      console.log('[Lobby] Using placeholder lobbies data');
      const data = PLACEHOLDER_LOBBIES.map(lobby => ({
        ...lobby,
        applicants: lobby.applicants || []
      }));
      console.log('[Lobby] Loaded lobbies:', data.length, 'lobbies');
      return data;
    } catch (error) {
      console.error('[Lobby] Error loading lobbies:', error);
  ...
  const loadLobbyProfilePictures = async (lobbiesToLoad) => {
    const picturePromises = lobbiesToLoad.map(async (lobby) => {
      ...
      try {
        ...
        console.error(`[Lobby] Error loading profile picture for lobby ${lobby.id}:`, error);
  ...
  const loadGames = async () => {
    try {
      console.log('[Lobby] Loading games...');
      // TODO: Replace with actual API call
      ...
      // For now, return placeholder data
      console.log('[Lobby] Using placeholder games data');
      const data = PLACEHOLDER_GAMES;
      console.log('[Lobby] Loaded games:', data.length, 'games');
      return data;
    } catch (error) {
  ...
  useEffect(() => {
    const loadCards = async () => {
      try {
        console.log('[Lobby] Loading cards from API...');
        setCardsLoading(true);
        const cards = await getCards();
        setCardsData(cards);
        console.log('[Lobby] Loaded cards:', cards.length, 'cards');
      } catch (error) {
        console.error('[Lobby] Error loading cards:', error);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log('[Lobby] Loading preferences and decks from API...');
        
        // Step 1: Migrate legacy decks if needed (only once)
        if (!hasMigratedRef.current) {
          console.log('[Lobby] Checking for legacy decks to migrate...');
          hasMigratedRef.current = true;
          await migrateLegacyDecks();
        }
        
        // Step 2: Load preferences from API
        console.log('[Lobby] Loading preferences from API...');
        const preferences = await getPreferences();
        console.log('[Lobby] Loaded preferences:', preferences);
    ...
    if (theme === 'dark') {
      ...
      document.documentElement.classList.remove('dark');
        ...
        console.log('[Lobby] Ensuring at least one deck exists...');
        await ensureOneDeck();
        
        // Step 4: Load decks from API
        console.log('[Lobby] Loading decks from API...');
        const loadedDecks = await getDecks();
    console.log('[Lobby] Loaded decks:', loadedDecks.map(d => ({ id: d.id, name: d.name })));
    setDecks(loadedDecks);
        setLoadingDecks(false);
    
        // Step 5: Set default deck from preferences
        const defaultId = preferences?.defaultDeckId || null;
        console.log('[Lobby] Default deck ID from preferences:', defaultId);
    setDefaultDeckIdState(defaultId);
        ...
        try {
          ...
          console.error('[Lobby] Error loading profile picture:', error);
        ...
        console.log('[Lobby] Loading lobbies and games...');
        ...
        console.log('[Lobby] Lobbies and games loaded successfully');
    ...
    if (!hasProcessedEditingDeckUUIDRef.current) {
      ...
      console.log('[Lobby] Editing deck UUID from storage:', editingDeckUUID);
      ...
      if (editingDeckUUID && loadedDecks.find(d => d.id === editingDeckUUID)) {
        ...
        console.log('[Lobby] Selecting editing deck UUID:', editingDeckUUID);
        ...
        console.log('[Lobby] Cleared editing deck UUID from storage');
      } else {
        ...
        if (defaultId && loadedDecks.find(d => d.id === defaultId)) {
          console.log('[Lobby] Selecting default deck:', defaultId);
          setSelectedDeckId(defaultId);
        } else if (loadedDecks.length > 0) {
          console.log('[Lobby] Selecting first deck:', loadedDecks[0].id);
          setSelectedDeckId(loadedDecks[0].id);
        }
      }
    }
      } catch (error) {
        console.error('[Lobby] Error loading data:', error);
        setLoadingDecks(false);
        ...
        try {
          ...
          console.error('[Lobby] Error in fallback:', fallbackError);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const updateTheme = async () => {
      try {
        console.log('[Lobby] Updating theme preference:', isDarkMode ? 'dark' : 'light');
        await updatePreferences({ theme: isDarkMode ? 'dark' : 'light' });
        setThemeLocal(isDarkMode ? 'dark' : 'light');
      } catch (error) {
        console.error('[Lobby] Error updating theme preference:', error);
      }
    };
  ...
  const reloadDecks = async () => {
    try {
      console.log('[Lobby] Reloading decks from API...');
      const loadedDecks = await getDecks();
      console.log('[Lobby] Reloaded decks:', loadedDecks.map(d => ({ id: d.id, name: d.name })));
      setDecks(loadedDecks);
    } catch (error) {
      console.error('[Lobby] Error reloading decks:', error);
    }
  };
  ...
  // Handle exit - return to homepage
  const handleExit = () => {
  ...
  const handleEditDeck = () => {
    if (selectedDeckId) {
      console.log('[Lobby] Edit button clicked for deck:', selectedDeckId);
      // Store the editing deck UUID so we can select it when returning to this page
      setEditingDeckUUID(selectedDeckId);
      console.log('[Lobby] Stored editing deck UUID:', selectedDeckId);
      window.location.href = `/deck/${selectedDeckId}`;
    } else {
      console.log('[Lobby] Edit button clicked but no deck selected');
    }
  };
  ...
  const handleSetAsDefault = async () => {
    if (selectedDeckId) {
      try {
        console.log('[Lobby] Setting default deck:', selectedDeckId);
      ...
      // Reload decks to update the star display
        ...
        console.error('[Lobby] Error setting default deck:', error);
      }
    }
  };
  ...
  const handleHostMatch = async () => {
    try {
      console.log('[Lobby] Creating lobby with settings:', hostForm);
      
      // Validate deck is usable for the selected format
      if (!isDeckUsableForFormat(hostForm.format)) {
        console.warn('[Lobby] Cannot host lobby: deck is not usable for format', hostForm.format);
        return;
      }
      ...
      console.log('[Lobby] Generated lobby UUID:', lobbyId);
      ...
      console.log('[Lobby] Created lobby object:', newLobby);
      ...
      setLobbies(prev => {
        ...
        console.log('[Lobby] Added lobby to list. Total lobbies:', updated.length);
        return updated;
      });
      ...
      setApplicants([]);
      console.log('[Lobby] Set hosted lobby ID:', lobbyId);
      console.log('[Lobby] Initialized applicants list (empty)');
  ...
  const handleCancelHost = async () => {
    try {
      if (!hostedLobbyId) {
        console.warn('[Lobby] Cannot cancel: no hosted lobby');
        return;
      }
      
      console.log('[Lobby] Cancelling hosted lobby:', hostedLobbyId);
      ...
      setLobbies(prev => {
        ...
        console.log('[Lobby] Removed lobby from list. Remaining lobbies:', filtered.length);
        return filtered;
      });
      ...
      setApplicants([]);
      console.log('[Lobby] Cleared hosted lobby state');
      console.log('[Lobby] Host form settings preserved:', hostForm);
      
    } catch (error) {
      console.error('[Lobby] Error cancelling lobby:', error);
    }
  };
  ...
  const handleJoinQueue = () => {
    ...
    if (hostedLobbyId) {
      console.log('[Lobby] Cannot join queue: user is currently hosting a lobby');
      return;
    }
  ...
  const handleJoinLobby = (lobby) => {
    // Prevent joining if user is currently hosting a lobby
    if (hostedLobbyId) {
      console.log('[Lobby] Cannot join lobby: user is currently hosting a lobby');
      return;
    }
  ...
  const joinLobbyWithPassword = (lobby, password) => {
    ...
    console.log('[Lobby] Joining lobby:', lobby.id, 'as', currentUsername, password ? 'with password' : 'without password');
  };
  ...
  const handleCancelJoinLobby = () => {
    ...
    console.log('[Lobby] Cancelled joining lobby:', joinLobbyModal.lobby.id);
  };
  
  // Handle refresh lobbies
  const handleRefreshLobbies = async () => {
    console.log('[Lobby] Refreshing lobbies...');
    try {
      const loadedLobbies = await loadLobbies();
      setLobbies(loadedLobbies);
      // Load profile pictures for lobbies
      await loadLobbyProfilePictures(loadedLobbies);
      console.log('[Lobby] Lobbies refreshed successfully:', loadedLobbies.length, 'lobbies');
    } catch (error) {
      console.error('[Lobby] Error refreshing lobbies:', error);
    }
  };
  
  // Handle refresh games
  const handleRefreshGames = async () => {
    console.log('[Lobby] Refreshing games...');
    try {
      const loadedGames = await loadGames();
      setGames(loadedGames);
      console.log('[Lobby] Games refreshed successfully:', loadedGames.length, 'games');
    } catch (error) {
      console.error('[Lobby] Error refreshing games:', error);
    }
  };
                        ...
                        applicants.map((applicant, index) => (
                          <div
                            ...
                            onClick={() => {
                              ...
                              console.log('[Lobby] Applicant clicked:', applicant);
                            }}
                          >
